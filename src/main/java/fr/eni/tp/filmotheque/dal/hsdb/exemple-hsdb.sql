--DROP DATABASE jdbc6_ENI;--Supprime la base si elle existe

--CREATE DATABASE jdbc6_ENI;--Crée la base si elle existe
...
ALTER TABLE COURS_FORMATEUR
    add constraint fk_FORMATEURS_COURS_FORMATEUR foreign key (email_formateur)
        references FORMATEURS(email);

ALTER TABLE COURS_FORMATEUR
    add constraint fk_COURS_ENI_COURS_FORMATEUR foreign key (id_cours)
